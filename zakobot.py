...
class SellingBtn(discord.ui.View): # Create a class called MyView that subclasses discord.ui.View
    ...
    async def button_callback(self, button, interaction):
        ...
        msg = f'✍️ A quest *{flavor1}**[{self.title} ({self.type})]({self.insertion})**{flavor2}* acabou de ser criada, está valendo ${str(self.reward)} e já está disponível no quadro!'
...
class ResenhaModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if dbservice.select('user', ['sexo'], '', {'id': self.user_id}) == 'm':
            msg = f'💪 A aventureira <@{str(self.user_id)}> completou e entregou a quest *{flavor1}**{self.item_name} ({self.type})**{flavor2}* criada por <@{str(sender_id)}>! A recompensa distribuída foi de ${str(self.buyer_reward)} e ${str(self.sender_reward)} respectivamente.'
        else:    
            msg = f'💪 O aventureiro <@{str(self.user_id)}> completou e entregou a quest *{flavor1}**{self.item_name} ({self.type})**{flavor2}* criada por <@{str(sender_id)}>! A recompensa distribuída foi de ${str(self.buyer_reward)} e ${str(self.sender_reward)} respectivamente.'
...
async def guilda_abandonar_quest_command(
    ...
    if exists == 1:
        ...
        if dbservice.select('user', ['sexo'], '', {'id': user_id}) == 'm':    
            ...
            msg = f'💀 O aventureiro <@{str(user_id)}> morreu tentando terminar a quest *{flavor1}**{obra} ({type})**{flavor2}* e a mesma foi devolvida ao quadro. Essa quest não conta para o limite de criação do criador.'
        await generate_guild_log(msg)
...
async def calculate_delivery_time(date_bought, quest_name, quest_type):
    ...
    if total_duration >= 2000:
        delivery_date = date_bought + (total_duration * 360)
    elif total_duration >= 1000:
        delivery_date = date_bought + (total_duration * 300)
...
async def sugerir_command(
    ...
    if exists == 0:
        dbservice.insert('gacha_candidate', ['id_chara', 'name', 'url', 'value', 'img'], [chara_id, chara_name, link, value, image])
    else:
